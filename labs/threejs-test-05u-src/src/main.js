...
const stateEl=$('state'),phaseEl=$('phase'),contactEl=$('contact'),jx=$('jx'),jy=$('jy'),jr=$('jr'),reactionEl=$('reaction'),syncEl=$('sync');
...
let jolt=null,attackEvent=null,contactCount=0;
...
const clock=new THREE.Clock();function animate(now){requestAnimationFrame(animate);const dt=Math.min(.033,clock.getDelta());player.mixer.update(dt);enemy.mixer.update(dt);updateAttack(now);updateCharacter(player,now);updateCharacter(enemy,now);applyCamera(now);renderer.render(scene,camera);}requestAnimationFrame(animate);
