...
import {ACTORS,BEHAVIOR,COMBAT,ENEMY_COMBAT,ENEMY_COMBAT_PHASE,WORLD} from '../config.js';
...
export class LivingWorldSystem{
  ...
  #writeVisual(slot,record){
    ...
    else if(record.enemyCombatPhase===ENEMY_COMBAT_PHASE.STRIKE)this.color.setHex(0xe13d2d);
    else if(record.enemyCombatPhase===ENEMY_COMBAT_PHASE.TELEGRAPH)this.color.setHex(0xe4a43a);
    else if(record.enemyCombatPhase===ENEMY_COMBAT_PHASE.RECOVERY)this.color.setHex(0x9d6542);
  ...
  stateSnapshot(id){
    ...
    return Object.freeze({
      ...
      defeated:r.defeated,
      hostileToPlayer:r.hostileToPlayer,
      threatLevel:r.threatLevel,
      threatUntilFrame:r.threatUntilFrame,
      enemyCombatPhase:r.enemyCombatPhase,
      enemyPhaseStartFrame:r.enemyPhaseStartFrame,
      enemyStrikeResolved:r.enemyStrikeResolved,
      enemyAttackCount:r.enemyAttackCount,
      enemyHitCount:r.enemyHitCount,
      enemyLastHitFrame:r.enemyLastHitFrame
    });
  }
  ...
  applyDamage(id,amount,frame){
    ...
    record.desiredHeading=null;
    record.awareness=1;
    record.hostileToPlayer=true;
    record.threatLevel=Math.min(1,(record.threatLevel||0)+.5);
    record.threatUntilFrame=frame+ENEMY_COMBAT.retaliationFrames;
    if(record.enemyCombatPhase===undefined)record.enemyCombatPhase=ENEMY_COMBAT_PHASE.READY;
    if(record.health<=0){
      ...
      record.enemyCombatPhase=ENEMY_COMBAT_PHASE.READY;
  ...
  snapshot(){
    ...
    return Object.freeze({
      ...
      hostileCount:this.activeRecords.filter(r=>r.hostileToPlayer&&!r.defeated).length,
      behavior,
      store
    });
  }
