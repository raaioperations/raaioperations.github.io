...
export class BehaviorSystem{
  ...
  evaluateRecord(record,frame,playerPosition,worldSpatialIndex){
    ...
    if(record.hostileToPlayer&&frame<=record.threatUntilFrame){
      const dx=playerPosition.x-record.x,dz=playerPosition.z-record.z;
      record.awareness=1;
      record.behavior=BEHAVIOR.OBSERVE_PLAYER;
      record.behaviorUntilFrame=Math.max(record.behaviorUntilFrame,frame+ACTORS.observeDurationFrames);
