...
export class PlayerController{
  constructor(root,{groundHeight=(x,z)=>0}={}){
    this.root=root;this.groundHeight=groundHeight;
    this.velocity=new THREE.Vector3();this.grounded=true;this.verticalVelocity=0;this.state='IDLE';this.speed=0;
    this.input={moveX:0,moveY:0,sprint:false,jump:false,lookDX:0,lookDY:0};
    this.fwd=new THREE.Vector3();this.right=new THREE.Vector3();this.desired=new THREE.Vector3();
  ...
  update({dt},cameraYaw){
    ...
    const lenSq=this.desired.lengthSq();
    if(lenSq>1)this.desired.normalize();

    const sprinting=this.input.sprint&&lenSq>.01;
    ...
    if(lenSq>.001){
      ...
      this.velocity.x=THREE.MathUtils.lerp(this.velocity.x,this.desired.x,blend);
      this.velocity.z=THREE.MathUtils.lerp(this.velocity.z,this.desired.z,blend);
    }else{
    ...
    const ground=this.groundHeight(this.root.position.x,this.root.position.z);
    ...
    if(this.speed>.12){
      ...
      let delta=((targetYaw-this.root.rotation.y+Math.PI)%(Math.PI*2))-Math.PI;
      this.root.rotation.y+=delta*(1-Math.exp(-14*dt));
    }
  ...
  snapshot(){
    const ground=this.groundHeight(this.root.position.x,this.root.position.z);
    return{
      state:this.state,speed:this.speed,grounded:this.grounded,
      x:this.root.position.x,y:this.root.position.y,z:this.root.position.z,
      groundY:ground,groundError:Math.abs(this.root.position.y-ground)
    };
  }
}
