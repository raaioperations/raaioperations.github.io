...
const out=path.resolve(root,'../threejs-test-11g');
const sourceRoot=path.join(root,'src');
const repoRoot=path.resolve(root,'../..');
const assert=(condition,message)=>{if(!condition)throw new Error('Test11G build proof failed: '+message);};
...
const baseline=JSON.parse(await readFile(path.resolve(root,'../threejs-test-11f-src/GREENFIELD_BASELINE.json'),'utf8'));
const status11F=JSON.parse(await readFile(path.resolve(root,'../threejs-test-11f-src/status.json'),'utf8'));
assert(status11F.status==='PASS_CLOSED','11F must be PASS/CLOSED');
assert(status11F.frozen===false,'11F test must remain unfrozen');
assert(baseline.name==='Greenfield Interactive World Foundation v0.6','v0.6 baseline required');
...
assert(sourceText.includes('class GameplayInteractionSystem'),'GameplayInteractionSystem required');
assert(sourceText.includes('class WorldInteractables'),'WorldInteractables required');
assert(sourceText.includes('selectInteractionTarget'),'deterministic target arbiter required');
assert(sourceText.includes("name:'gameplay-interaction'"),'gameplay interaction scheduler phase required');
assert(sourceText.includes("KeyE"),'keyboard E interaction input required');
assert(sourceText.includes("data-use"),'touch USE control required');
assert(sourceText.includes('actionBudgetPerFrame:1'),'one action/frame budget required');
assert(sourceText.includes("PICKUP"),'pickup action required');
assert(sourceText.includes("NPC"),'NPC interaction action required');
assert(sourceText.includes('playerInteractionCount'),'persistent NPC interaction state required');
assert(sourceText.includes('pickupCollected'),'pickup persistence telemetry required');
assert(sourceText.includes('waystoneToggled'),'USE state telemetry required');
...
const buildInfo={
  ...
  milestone:'Gameplay Interaction Foundation',
  environment_name:'Copperwash Reach — Gameplay Interaction',
  baseline:'Greenfield Interactive World Foundation v0.6',
  architecture:'GREENFIELD_GAMEPLAY_INTERACTION_FOUNDATION',
  ...
  capabilities:{
    deterministic_target_arbitration:true,
    context_prompt:true,
    keyboard_interaction_key:'E',
    touch_interaction_button:'USE',
    interaction_action_budget_per_frame:1,
    pickup_state:true,
    use_toggle_state:true,
    npc_interaction_initiation:true,
    npc_interaction_state_persistence:true,
    active_actor_capacity:27,
    world_chunk_pool:9
  },
  limits:{draw_calls_max:78,triangles_max:105000,raf_loops:1},
  assets:copiedAssets,
...
await writeFile(path.join(out,'verification-report.json'),JSON.stringify({
  ...
  build_id:buildId,
  checks:{
    test11f_pass_closed:true,
    baseline_v06:true,
    one_raf_preserved:true,
    no_set_animation_loop:true,
    target_arbitration:true,
    prompt_system:true,
    keyboard_touch_interact:true,
    one_action_per_frame:true,
    pickup_state:true,
    use_state:true,
    npc_interaction:true,
    interaction_state_persistence:true,
