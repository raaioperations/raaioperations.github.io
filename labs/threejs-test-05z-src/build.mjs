...
try{
  ...
  d=replaceUnique(d,"targetHP=Math.max(0,targetHP-ATTACK_DAMAGE);targetHpEl.textContent=targetHP+'/100';","targetHP=Math.max(0,targetHP-ATTACK_DAMAGE);triggerDefaultMeleeImpact(performance.now());targetHpEl.textContent=targetHP+'/100';",'25-damage default impact wiring');
  d=replaceUnique(d,"if(mixer)mixer.update(dt*(sprinting?1.08:1));updateAttack(dt);","const impactDt=updateDefaultMeleeImpact(dt,performance.now());if(mixer)mixer.update(impactDt*(sprinting?1.08:1));updateAttack(impactDt);",'local hitstop update hook');
  m=replaceUnique(m,"counterDamageEvents++;\n  uiText('counterTargetHp',counterTargetHP+'/100');","counterDamageEvents++;\n  triggerCameraJolt(40);\n  uiText('counterTargetHp',counterTargetHP+'/100');",'40-damage accepted counter jolt wiring');
  ...
  console.log('05Z diagnostic: building modified 05D source baseline directly...');
  execFileSync('npm',['run','build'],{cwd:path.join(labs,'threejs-test-05d-src'),stdio:'inherit'});
  console.log('05Z diagnostic: modified 05D source baseline PASS; continuing full lineage...');
  ...
  info.build_id=buildId;info.test='05Z';info.focus='real successful 25-damage combat event drives accepted/default ordinary-melee presentation stack';info.visual_runtime_revision='05Z authoritative 25-damage hit → default impact presentation';
  info.default_melee_impact={source_level:true,trigger:'successful accepted 25-damage player attack event only',damage:25,micro_jolt:{duration_ms:82,amp_x_px:1.65,amp_y_px:.9,rotation_deg:.032,zoom:.0008,decay_power:2.05,phase_multiplier:2.8},hit_reaction:{duration_ms:145,peak_ms:42,recoil_m:.052,lean_deg:2.35,contact_pose_preload_ms:18},hitstop:{duration_ms:24,scope:'attacker animation + attack progression; world/controller time remains live',camera_continues:true},knockback:{distance_m:.07,duration_ms:100,persistent_world_displacement:true,direction:'away from attacker'},contact_burst:{duration_ms:92,ring_inner_m:.075,ring_outer_m:.105,ray_count:8,ray_length_m:.14,world_space:true,screen_flash:false},miss_feedback:false,max_events_per_successful_contact:1};
  info.strong_counter_impact={damage:40,jolt:40,accepted_behavior_retained:true,default_melee_layers_added:false};info.jolt25={accepted_diagnostic_profile_retained:true,basic_25_damage_hit_uses_it:false,reserved_for_future_heavier_profile:true};info.post_bundle_camera_patch=false;info.human_acceptance={accepted:false,status:'PENDING HUMAN REAL-COMBAT IMPACT REVIEW'};info.acceptance_checklist={real_25_damage_hit_triggers_complete_default_stack:'HUMAN REVIEW',miss_has_no_default_feedback:'DELEGATED PASS',two_hit_chain_triggers_one_default_event_per_successful_contact:'HUMAN REVIEW',real_40_damage_counter_retains_jolt40_only:'DELEGATED PASS',accepted_combat_and_telegraph_readability_remain_intact:'HUMAN REVIEW'};info.service_worker_cache=false;
  await writeFile(path.join(out,'build-info.json'),JSON.stringify(info,null,2));

  const verification={test:'05Z',status:'PASS',build_id:buildId,delegated_nonvisual_checks:{accepted_05q_required:true,accepted_05r_v2_required:true,accepted_05y_default_required:true,default_impact_source_level:true,normal_hit_wired_after_authoritative_25_damage_mutation:true,miss_path_has_no_default_impact_call:true,local_hitstop_duration_ms:24,local_hitstop_does_not_freeze_camera_or_world_controller_time:true,persistent_knockback_distance_m:.07,contact_burst_successful_hit_only:true,counter_40_still_calls_accepted_jolt40:true,default_layers_not_added_to_counter:true,post_bundle_camera_patch:false},human_visual_review:'REQUIRED'};
