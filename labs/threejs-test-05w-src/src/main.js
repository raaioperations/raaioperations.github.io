...
const stateEl=$('state'),phaseEl=$('phase'),contactEl=$('contact'),hitstopEl=$('hitstop'),knockEl=$('knockback'),distanceEl=$('distance'),jx=$('jx'),jy=$('jy'),jr=$('jr'),reactionEl=$('reaction'),syncEl=$('sync');
...
function setupCharacter(gltf,x,yaw){const root=new THREE.Group();root.position.set(x,0,0);root.rotation.y=yaw;scene.add(root);const model=gltf.scene;root.add(model);model.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}});const mixer=new THREE.AnimationMixer(model);const idle=gltf.animations.find(c=>c.name==='Idle')||gltf.animations[0];if(idle)mixer.clipAction(idle).play();return{root,model,mixer,baseX:x,baseYaw:yaw,worldOffsetX:0,reaction:null,attack:null,knockback:null};}
const player=setupCharacter(aGLB,-.85,-Math.PI/2),enemy=setupCharacter(bGLB,.85,Math.PI/2);

// Accepted 05T-R2 + current 05V feedback constants remain unchanged.
...
// New isolated variable under review: small persistent world-space displacement of the receiver.
...
function resetPositions(){if(attackEvent)return;for(const c of [player,enemy]){c.worldOffsetX=0;c.knockback=null;c.reaction=null;c.attack=null;}knockEl.textContent='0.00 m';contactEl.textContent='WAIT';reactionEl.textContent='IDLE';phaseEl.textContent='READY';stateEl.textContent='READY';stateEl.style.color='#9df2ae';updateDistance();}
...
function updateCharacter(c,time){
  ...
  if(c.attack){const p=Math.min(1,(time-c.attack.start)/MELEE.total);const contactP=MELEE.windup/MELEE.total;const toContact=Math.min(1,p/contactP);const recovery=Math.max(0,(p-contactP)/(1-contactP));const e=p<contactP?Math.sin(toContact*Math.PI/2):Math.cos(Math.min(1,recovery)*Math.PI/2);c.root.position.x+=c.attack.direction*REACTION.lunge*e;c.root.rotation.z=-c.attack.direction*THREE.MathUtils.degToRad(.9)*e;if(p>=1)c.attack=null;}
...
function doContact(wallNow){const give=attackEvent.give,receiver=give?enemy:player,direction=give?1:-1;const distance=Math.abs(anchorX(enemy)-anchorX(player));attackEvent.contactDone=true;phaseEl.textContent='CONTACT';if(distance>MELEE.reach){contactEl.textContent='MISS';return;}const joltStart=wallNow,reactionStart=simNow;beginJolt(direction,joltStart);beginReaction(receiver,direction,reactionStart);beginHitstop();beginKnockback(receiver,direction,reactionStart);contactCount++;contactEl.textContent='HIT '+contactCount;reactionEl.textContent=give?'TARGET HIT':'PLAYER HIT';syncEl.textContent='0 ms';}
...
$('give').addEventListener('click',()=>startMelee(true));$('receive').addEventListener('click',()=>startMelee(false));$('reset').addEventListener('click',resetPositions);addEventListener('keydown',e=>{if(e.repeat)return;if(e.code==='KeyM'){e.preventDefault();startMelee(true);}else if(e.code==='KeyN'){e.preventDefault();startMelee(false);}else if(e.code==='KeyP'){e.preventDefault();resetPositions();}});
const clock=new THREE.Clock();function animate(wallNow){requestAnimationFrame(animate);const wallDt=Math.min(.033,clock.getDelta());let simDt=wallDt;if(hitstopRemaining>0){hitstopRemaining=Math.max(0,hitstopRemaining-wallDt*1000);simDt=0;hitstopEl.textContent=Math.ceil(hitstopRemaining)+' ms';if(hitstopRemaining===0)hitstopEl.textContent='0 ms';}simNow+=simDt*1000;player.mixer.update(simDt);enemy.mixer.update(simDt);updateAttack(wallNow);updateCharacter(player,simNow);updateCharacter(enemy,simNow);updateDistance();applyCamera(wallNow);renderer.render(scene,camera);}requestAnimationFrame(animate);
