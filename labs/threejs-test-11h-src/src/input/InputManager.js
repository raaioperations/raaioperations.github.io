export class InputManager{
  constructor({canvas,container,coarsePointer=false}){
    ...
    this.keys=new Set();this.moveX=0;this.moveY=0;this.sprint=false;this.jumpQueued=false;this.interactQueued=false;this.attackQueued=false;this.lookDX=0;this.lookDY=0;this.dragPointer=null;this.testIntent=null;
    this.onKeyDown=e=>{
      this.keys.add(e.code);
      if(e.code==='Space'){this.jumpQueued=true;e.preventDefault();}
      if(e.code==='KeyE'){this.interactQueued=true;e.preventDefault();}
      if(e.code==='KeyF'){this.attackQueued=true;e.preventDefault();}
    };
  ...
  #buildTouchControls(){
    ...
    this.ui.innerHTML='<div class="stick" data-stick><div class="knob" data-knob></div></div><button class="touch-btn sprint" data-sprint>RUN</button><button class="touch-btn jump" data-jump>JUMP</button><button class="touch-btn use" data-use>USE</button><button class="touch-btn attack" data-attack>ATTACK</button>';
    ...
    this.attackBtn=this.ui.querySelector('[data-attack]');
    ...
    this.onAttack=e=>{e.preventDefault();this.attackQueued=true;};
    this.attackBtn.addEventListener('pointerdown',this.onAttack);
  }
  ...
  dispose(){
    ...
    this.attackBtn?.removeEventListener('pointerdown',this.onAttack);
    this.ui?.remove();
  }
}
