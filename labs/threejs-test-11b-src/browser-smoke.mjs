...
const url=process.env.TEST_URL||'http://127.0.0.1:4174/?smoke=1';
...
const expression="(()=>({smoke:document.documentElement.dataset.browserSmoke||'PENDING',runtimeError:document.documentElement.dataset.runtimeError||'?',renderer:document.documentElement.dataset.renderer||'',soldier:document.documentElement.dataset.soldier||'',movement:document.documentElement.dataset.movement||'',jump:document.documentElement.dataset.jump||'',camera:document.documentElement.dataset.camera||'',frameCount:document.documentElement.dataset.frameCount||'',drawCalls:document.documentElement.dataset.drawCalls||'',triangles:document.documentElement.dataset.triangles||'',text:document.body?.innerText?.slice(0,1200)||''}))()";
const start=Date.now();
let last=null;
...
cdp.ws.close();
if(!last)throw new Error('No browser smoke state returned');
if(last.smoke!=='PASS')throw new Error('11B browser smoke did not pass');
if(last.runtimeError!=='0')throw new Error('11B runtime error flag set');
if(last.renderer!=='WebGLRenderer')throw new Error('11B renderer mismatch');
if(last.soldier!=='PASS')throw new Error('Soldier GLB did not pass');
if(last.movement!=='PASS')throw new Error('Movement proof did not pass');
if(last.jump!=='PASS')throw new Error('Jump proof did not pass');
